...
$(document).ready(function(){
    if(unbxdAutoSuggestSiteKey && unbxdAutoSuggestApiKey && $("#"+unbxdAutoSuggestSearchInputId)) {
        ...
        window.searchobj = new window.Unbxd.setSearch({
            ...
            searchResultSetTemp: {
                "grid": ['{{#each (productVariant products)}}<li class="product-item"><a href="/yacceleratorstorefront{{url}}" title="{{{name.[0]}}}" id="{{sku}}" class="thumb" unbxdParam_sku="{{uniqueId}}" unbxdParam_pRank="{{unbxdprank}}" unbxdAttr="product">',
                    '<img id="img-{{uniqueId}}" src="/yacceleratorstorefront{{{[img-300Wx300H]}}}" alt="{{{name.[0]}}}" title="{{{name.[0]}}}"></a>',
                    '<div class="details">',
                    '<a class="name" href="/yacceleratorstorefront{{url}}">{{name.[0]}}</a>',
                    '{{#isSwatches}}',
                    ...
                    '<button class="btn btn-default btn-block js-pickup-in-store-button glyphicon glyphicon-map-marker" id="product_{{code}}" type="button submit" data-productcart="{{#if (isdefined priceValue) }}&pound;{{priceValue}}{{else}}&pound;0.00{{/if}}" data-productcart-variants="{}" data-img-html="<img src=&quot;/yacceleratorstorefront{{{[img-300Wx300H]}}}&quot;/>" data-productname-html="{{{name.[0]}}}"  data-cartpage="false" data-entrynumber="{{@index}}" data-actionurl="/yacceleratorstorefront{{url}}" data-value="1"></button>',
                    '</div>',
                    ...
                    '<input type="hidden" name="productNamePost" value="{{{name.[0]}}}">',
                ...
                "list": ['{{#products}}<li class="product__list--item">',
                    ...
                    '<img src="/yacceleratorstorefront{{{[img-300Wx300H]}}}" alt="{{{name.[0]}}} title="{{{name.[0]}}}">',
                    '</a>',
                    '<a href="/yacceleratorstorefront{{url}}" id="{sku}}" class="product__list--name"  unbxdParam_sku="{{uniqueId}}" unbxdParam_pRank="{{unbxdprank}}" unbxdAttr="product">', '{{{name.[0]}}}', '</a>',
